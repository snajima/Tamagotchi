...
let elementals_test =
  ...
  [
    ...
    our_opponent_test "our play water then fire"
      (fun () ->
        init_game () |> play_water |> next |> play_fire |> get_ours)
      (Fire, 20);
    our_opponent_test "our play fire then leaf"
      (fun () ->
        init_game () |> play_fire |> next |> play_leaf |> get_ours)
      (Leaf, 20);
    our_opponent_test "our play leaf then water"
      (fun () ->
        init_game () |> play_leaf |> next |> play_water |> get_ours)
      (Water, 20);
    ...
    our_opponent_test "initial opponent - Water"
      ...
      (Water, 100);
    our_opponent_test "initial opponent - Leaf"
      (fun () -> init_game () |> get_opponent)
      (Leaf, 100);
    our_opponent_test "initial opponent - Fire"
      (fun () -> init_game () |> get_opponent)
      (Fire, 100);
    (* --------------------- Play Something First ---------------------- *)
    our_opponent_test "opponent play water"
    ...
    win_loss_test "wins play water"
      (fun () -> init_game () |> play_water |> next |> get_wins)
      1;
    win_loss_test "wins play fire"
      (fun () -> init_game () |> play_fire |> next |> get_wins)
      1;
    win_loss_test "wins play leaf"
      (fun () -> init_game () |> play_leaf |> next |> get_wins)
      1;
    (* -------------------- Play Something Second --------------------- *)
    ...
    win_loss_test "loss play water"
      (fun () -> init_game () |> play_water |> next |> get_losses)
      1;
    win_loss_test "loss play fire"
      (fun () -> init_game () |> play_fire |> next |> get_losses)
      1;
    win_loss_test "loss play leaf"
      (fun () -> init_game () |> play_leaf |> next |> get_losses)
      1;
    (* -------------------- Play Something Second --------------------- *)
    ...
    currently_animated_test "currently animated - play next"
      (fun () ->
        init_game () |> play_water |> next |> get_currently_animated)
      false;
    (* ----------------------- Don't Play Next ------------------------ *)
