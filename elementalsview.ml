...
let elementals_except s ex =
  ...
  | Elementals.Gameover w_l ->
    if w_l then (
      draw_pixels (default_vs.maxx / 2) (default_vs.maxy / 2) default_vs.maxx default_vs.maxy white;
      draw_message ((default_vs.maxx * default_vs.scale) / 2) ((default_vs.maxy * default_vs.scale) - 40) 50 red ((string_of_int 2) ^ " - " ^ (string_of_int (get_losses g.game)));
      ...
      draw_message ((default_vs.maxx * default_vs.scale) / 2) ((default_vs.maxy * default_vs.scale) - 40) 50 red ((string_of_int (get_wins g.game)) ^ " - " ^ (string_of_int 2));
...
let rec get_elems_anims
  ...
  | (Leaf, height) :: t ->
    get_elems_anims t
      ...
      :: lst_so_far)
  | (Nothing, _) :: t -> 
    get_elems_anims t lst_so_far

let robot_and_tamagotchi =
  [
    {robot_anim with cx = 105;
                     cy = default_vs.maxy / 2;};
    {shoot_anim with cx = 15;
                     cy = default_vs.maxy / 2;}
  ]

let get_animations (game : Elementals.gamestate) : Animation.animation list =
  get_elems_anims ((get_ours g.game) :: [get_opponent g.game]) robot_and_tamagotchi
...
let elementals_predraw s =
  draw_pixels (default_vs.maxx / 2) (default_vs.maxy / 2)
  default_vs.maxx default_vs.maxy white;
  draw_message ((default_vs.maxx * default_vs.scale) / 2) ((default_vs.maxy * default_vs.scale) - 40) 50 red ((string_of_int (get_wins g.game)) ^ " - " ^ (string_of_int (get_losses g.game)))

let draw () =
  draw_loop vs elementals_init elementals_exit elementals_key elementals_except
  elementals_step elementals_predraw