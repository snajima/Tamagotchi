...
(** The type [color] represents the two possible colors of the beats
    in the game. *)
type color =
  | Don
  | Ka
...
(** The type [gamestate] represents state of the Drum game simulation
  ...
  - Good multipler (i.e. how many more points you get when you hit
  a good note vs an "ok" one)
  - A combo multipler (i.e. how much your store exponentially increases
  when you have a high combo)
...
(** The type [hit] represents state of the type of the hit per
  beat 
  - Note that the OutOfRange is to not punish players for hitting a button
...
(** [beats] is a list of beats and their locations. They are hard-coded
  beats that are randomly chosen from when calling [add_beat]. The reason
...
(** [get_beats gs] returns a list of coordinates that indicate the
    ...
    Color is of type [color], and indicates if the beat is a "Don" or "Ka".
    Height is a integer between 1 - [max_height] *)
val get_beats : gamestate -> (int * color) list

(** [get_num_beats gs] returns the number of beats left until
  the game is over. *)
...
(** [add_rock gs] randomly selects a beat pattern in [beats] and adds
    it to the game with [max_height + 1]. Note that the
    beats only enters the game simulation when next is called to have
    consistent behaviour -- all beats fall for the same number of turns *)
