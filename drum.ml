...
let beats = [
  [( max_height + 1, Don )];
  [( max_height + 1, Ka )];
  [( max_height + 1, Don ); ( max_height + 16, Don ); ( max_height + 31, Don )];
  [( max_height + 1, Ka ); ( max_height + 16, Ka ); ( max_height + 31, Ka )];
  [( max_height + 1, Don ); ( max_height + 16, Ka ); ( max_height + 31, Don )];
...
let init_game () : gamestate =
  { combo = 0; score = 0; beats = []; beat_type = Idle; good_multiplier = 1.6; combo_multiplier = 10; player_x = 10; good_range = 2; ok_range = 4; bad_range = 5; game_length = 10; num_beats = 11 }
